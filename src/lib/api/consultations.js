...
/**
 * ⚖️ ক্লায়েন্টের ইমেইল অ্যাড্রেস দিয়ে তার পূর্ববর্তী সমস্ত হায়ার বা কনসালটেশন হিস্ট্রি নিয়ে আসার এপিআই
 */
export async function getConsultationsByClient(clientEmail) {
  try {
    if (!clientEmail) return [];
    
    // 🔐 ২য় প্যারামিটার `true` পাস করা হয়েছে, কারণ ব্যাকঅ্যান্ডে এই রুটটি verifyToken দিয়ে প্রটেক্টেড
    return await serverFetch(`/api/lawyer/hirings/${clientEmail.toLowerCase().trim()}`, true);
  } catch (error) {
    console.error(`Error fetching consultations for client ${clientEmail}:`, error);
    return [];
  }
