...
export async function getPlanById(planId) {
  try {
    ...
    // ব্যাকএন্ডের /api/plans পাথের সাথে কুয়েরি প্যারামিটার যুক্ত করে কল করা হলো (Public Access)
    return await serverFetch(`/api/plans?plan_id=${targetId}`);
  } catch (error) {
    console.error(`Error fetching plan for ID ${planId}:`, error);
...
export async function getLawyerUsage(email) {
  try {
    ...
    // 🔐 ব্যাকএন্ডের এই সিকিউর রাউটটি চেক করার জন্য ২য় প্যারামিটার true পাস করা হলো
    return await serverFetch(`/api/lawyer/usage/${email}`, true);
  } catch (error) {
    console.error("❌ Error inside getLawyerUsage API helper:", error);
