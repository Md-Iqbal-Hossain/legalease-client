...
//     // ৩. ডাটা ফেচিং এবং কোটা ক্যালকুলেশন
//     const consultations = await getConsultationsByClient(user.id);
//     const plan = await getPlanById(user?.plan || 'client_free');
//     const lawyer = await getLawyerById(id);
...
const LegalHirePage = async ({ params }) => {
    ...
    // ৩. এক্সপ্রেস ব্যাকএন্ড থেকে লাইভ হায়ারিং ও পেমেন্ট ডাটা ফেচিং
    let consultationCount = 0;
    try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_API_URL}/api/lawyer/hirings/${user.email}`, {
            cache: 'no-store' // ক্যাশিং সম্পূর্ণ ডিজেবল রাখা হলো লাইভ আপডেটের জন্য
        });
        if (res.ok) {
            const hiringsData = await res.json();
            if (Array.isArray(hiringsData)) {
                // 🎯 ডাটাবেজ থেকে শুধুমাত্র সফলভাবে পেমেন্ট করা ('Paid') কেসগুলো ফিল্টার করে কাউন্ট করা হলো
                const paidCases = hiringsData.filter(item => item.paymentStatus === 'Paid');
                consultationCount = paidCases.length;
            }
        }
    } catch (error) {
        console.error("❌ Failed to stream client consultation quota from Express backend:", error);
    }

    // ৪. প্ল্যান ও লয়ার ডাটা রেজোলিউশন
    const plan = await getPlanById(user?.plan || 'client_free');
    const lawyer = await getLawyerById(id);

    // কোটা লিমিট এবং লিমিট রিচড লজিক
    const maxConsultations = plan?.maxConsultationsPerMonth || 3; // ব্যাকআপ হিসেবে ৩ রাখা হলো
    const hasReachedLimit = consultationCount >= maxConsultations;
    
    // প্রগ্রেস বারের পার্সেন্টেজ ক্যালকুলেশন
    const usagePercentage = Math.min((consultationCount / maxConsultations) * 100, 100);
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-lg">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                        <div>
                            ...
                            <h2 className="text-lg font-bold text-zinc-100 mt-0.5">
                                Used <span className="text-amber-400 font-extrabold">{consultationCount}</span> out of <span className="text-zinc-400">{maxConsultations}</span> standard requests
                            </h2>
                        </div>
                        <span className="self-start sm:self-center px-2.5 py-1 text-xs font-medium rounded-full bg-zinc-800 text-zinc-300 border border-zinc-700">
                            Tier: <strong className="text-amber-400 font-semibold">{plan?.name || 'Free Tier'}</strong>
                        </span>
                    </div>

                    {/* ডায়নামিক প্রграфии বার */}
                    <div className="w-full bg-zinc-800 h-2.5 rounded-full overflow-hidden mb-5">
                        <div 
                            className={`h-full transition-all duration-500 rounded-full ${
                                hasReachedLimit ? 'bg-red-500' : usagePercentage > 75 ? 'bg-amber-500' : 'bg-amber-400'
